...
while(!eof(f_File)){
	...
	# Retrieve sequences #
	######################
        $seqh = $dbh -> prepare(qq(SELECT mel, sim_alleles as sim
                                   FROM tfbs_poly
                                   WHERE coordinate between $fp_start - $flank and $fp_end + $flank));
        $seqh -> execute();
	(%seq) = ();
	(@sim) = ();
        while(($mel, $sim_alleles) = $seqh -> fetchrow_array){
	    $seq{'mel'} .= $mel;
	    @sim = split(/\|/, $sim_alleles);
	    $i = 1;
	    foreach (@sim){
		$seq{$species[$i]} .= $_;
		$i ++;
	...
	if($i == 7 && $num_alleles <= 2){
	    ...
	    print f_Output "$tfbs_id\t\t$region\t$factor\t$score[0]\t$alleles\t$loss\t$shift\t$shift_freq\t$status\t$scorechange\t$scorechange_freq\n";
