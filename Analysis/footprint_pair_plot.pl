################################
# perl footprint_pair_plot.pl
# Created Jun 17th 08
# To plot energy pairs for 
# footprint sites in 
# mel and sim
# Hebin
################################
...
$sth0 = $dbh -> prepare(qq(select region, tfbs_start, tfbs_end from tfbs where factor = '$factor'));
$sth0 -> execute;
$sth0 -> bind_columns(\$region,\$start, \$end);
while($sth0 -> fetch){
    if(!defined($region)){
	next;
    }
    $sth1 = $dbh->prepare(qq(select anc, mel, sim_alleles
			     from tfbs_poly
			     where region = '$region' and coordinate between $start and $end));
    $sth1->execute;
    (@array) = (); $mel = $anc = $sim = '';
