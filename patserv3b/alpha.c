/* Copyright 1990, 1992, 1994, 1996 Gerald Z. Hertz
 ...
 * The following external variables are required:
    Alpha_file;     / The name of the alphabet file. /
    Comp_status;    / Flag for whether to scan complementary strand.
                    / 0 means no. /
    Ascii;          / Flag for whether alphabet characters are
                    / ASCII (YES) or integer (NO). /
    Case_sensitive; / Flag for whether letter alphabet is case sensitive.
		    /     0: case sensitive; 1: not case sensitive;
		    /     2: not case sensitive, but indicate the locations
		    /        of lowercase letters. /
    Comp_flag;      / Flag indicating whether alphabet is complementary and
		    / whether any letters are their own complement.
		    /     0: no complements; 1: no own; 2: some own /
    A_size;         / Alphabet size---i.e., number of letters. /
    A;              / Translation between an integral index and the
                    / corresponding letter/integer.  Convention for nucleic
                    / acid alphabets: if "n" is the index of a letter,
                    / its complement will have the index (n+1), if the
                    / letters are not the same. /
    A_comp;         / Translation between the index of a letter and
		    / the index of its complement. /
    P;              / The prior frequencies of the letters /
 */
...
/* The following three functions are declared in "alpha.h" and
...
extern int pl_String();      /* "parse-line.c" function for reading strings. */
 ...
 */
static int ALPHA_read_letter(argc, argv, i, k)
 ...
 */
static double ALPHA_read_number(argc, argv, i, k)
     int argc;           /* Number of variables on the command line. */
     char *argv[];       /* The table of command line strings. */
     int *i;             /* The index to the current command line string. */
     int *k;             /* Index to current position of the current string. */
 ...
 *     EOF: colon NOT found.
 */
static int ALPHA_read_colon(argc, argv, i, k)
     int argc;           /* Number of variables on the command line. */
     char *argv[];       /* The table of command line strings. */
     int *i;             /* The index to the current command line string. */
     int *k;             /* Index to current position of the current string. */
...
static int ALPHA_alloc()
{
  int array_size = ALPHA_SIZE;   /* Determine the array_size. */


  A = (int *)calloc_error(array_size, sizeof(int), "A", "ALPHA_alloc()");

  A_comp = (int *)calloc_error(array_size, sizeof(int),
			       "A_comp", "ALPHA_alloc()");

  P = (double *)calloc_error(array_size, sizeof(double), "P", "ALPHA_alloc()");

  return(array_size);
}

/* Reallocate space for "A[]", "A_comp[]", and "P[]". */
static int ALPHA_realloc(array_size)
     int array_size;
{
  /* Determine the new array_size. */
  array_size = array_size + ALPHA_SIZE;

  A = (int *)recalloc_error((char *)A, array_size, sizeof(int),
			    "A", "ALPHA_realloc()");

  A_comp = (int *)recalloc_error((char *)A_comp, array_size, sizeof(int),
				 "A_comp", "ALPHA_realloc()");

  P = (double *)recalloc_error((char *)P, array_size, sizeof(double),
			       "P", "ALPHA_realloc()");
...
static void ALPHA_truncate()
{
  A = (int *)recalloc_error((char *)A, A_size, sizeof(int),
			    "A", "ALPHA_truncate()");

  if (Comp_flag == 0)
    free_error((char *)A_comp, "A_comp", "ALPHA_truncate()");
  else
    A_comp = (int *)recalloc_error((char *)A_comp, A_size, sizeof(int),
				   "A_comp", "ALPHA_truncate()");

  P = (double *)recalloc_error((char *)P, A_size, sizeof(double),
			       "P", "ALPHA_truncate()");
}
 ...
 */
static int ALPHA_get_letter(fp)
     FILE *fp;
{
 ...
 */
static double ALPHA_get_double(fp)
     FILE *fp;
{
 ...
 */
static int ALPHA_get_int(fp, number)
     FILE *fp;
...
int pl_Alpha_af(variable, argc, argv, i, k)
     ...
     int k;              /* Index to current position of the current string. */
{
  STalpha_flag = 1;
  Ascii = 1;
  return(pl_String((char *)&Alpha_file, argc, argv, i, k));
}

/* The name of the file from which to read the integer alphabet. */
int pl_Alpha_if(variable, argc, argv, i, k)
     char *variable;     /* Address of the variable to be updated. */
     int argc;           /* Number of variables on the command line. */
     char *argv[];       /* The table of command line strings. */
     int i;              /* The index to the current command line string. */
...
int pl_Alpha_ac(variable, argc, argv, i, k)
     char *variable;     /* Address of the variable to be updated. */
     int argc;           /* Number of variables on the command line. */
     char *argv[];       /* The table of command line strings. */
     int i;              /* The index to the current command line string. */
     int k;              /* Index to current position of the current string. */
{
   ...
   *     EOF: no alpha letter was found.
   */
  int ALPHA_read_letter();      /* arguments: argc, argv, &i, &k */
   ...
   *     0.0: positive number was not found.
   */
  double ALPHA_read_number();      /* arguments: argc, argv, &i, &k */
   ...
   *     EOF: colon NOT found.
   */
  int ALPHA_read_colon();       /* arguments: argc, argv, &i, &k */
...
static void ALPHA_read_int()
{
   ...
   *     EOF: EOF reached.
   */
  long find_line();      /* arguments: fp, Alpha_file */

  /* Read the next integer character in the file.  Returns values:
   *     YES: an integer was found;
   *     EOF: an integer was not found.
   */
  int ALPHA_get_int();         /* arguments: fp, *number */
   ...
   *     0.0: positive number was not found.
   */
  double ALPHA_get_double();      /* arguments: fp */
   ...
   *     EOF: colon NOT found.
   */
  int ALPHA_get_colon();       /* arguments: fp */
...
static void ALPHA_read_ascii()
{
   ...
   *     EOF: not an alpha letter.
   */
  int ALPHA_get_letter();      /* arguments: fp */
