...
void print_directions()
{
  ...
  void text_directions();      /* The text of the directions. */
...
{
  fprintf(fp, "Copyright 1990, 1994, 1995, 1996 Gerald Z. Hertz\nMay b");
  fprintf(fp, "e copied for noncommercial purposes.\n\n");

  fprintf(fp, "Author:\n  Gerald Z. Hertz\n  Dept. of Molecular, Cellu");
  fprintf(fp, "lar, and Developmental Biology\n  University of Colorad");
  fprintf(fp, "o\n  Campus Box 347\n  Boulder, CO  80309-0347\n\n");

  fprintf(fp, "  hertz@boulder.colorado.edu\n\n");

  fprintf(fp, "\n");

  fprintf(fp, "\n");

  fprintf(fp, "PATSER (version 3b)\n\n");
  ...
  fprintf(fp, "ertz et al., 1990, CABIOS, 6:81-92).\nA weight matrix i");
  fprintf(fp, "s a matrix whose elements are in a form\nconsidered app");
  fprintf(fp, "ropriate for scoring an L-mer.\n\n");
  ...
  fprintf(fp, "  5) Options for adjusting or restricting which scores ");
  fprintf(fp, "are printed.\n     -c: also score the complementary seq");
  fprintf(fp, "uences.  The complements are\n        determined by the");
  fprintf(fp, " program and are not explicitly stated in the\n        ");
  fprintf(fp, "sequence files.\n     -l number: lower threshold for pr");
  fprintf(fp, "inting scores, inclusive.\n     -u number: upper thresh");
  fprintf(fp, "old for printing scores, exclusive.\n     -t: just prin");
  fprintf(fp, "t the top score for each sequence.\n\n");
