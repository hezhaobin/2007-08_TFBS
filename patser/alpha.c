/* Copyright 1990, 1992, 1994, 1996, 1997, 1998, 2001 Gerald Z. Hertz
 ...
 *    description of the "parse_line" vector and any other files using
 *    the alphabet variables listed below.
...
char Comp_status = 0;    /* Flag for whether to scan complementary strand.
     ...
     int array_size)             /* The amount of space currently allocated. */
{
  ...
  A = (int *)recalloc_error(A, array_size, sizeof(int),
			    "A", "ALPHA_realloc()");

  A_comp = (int *)recalloc_error(A_comp, array_size, sizeof(int),
				 "A_comp", "ALPHA_realloc()");

  P = (double *)recalloc_error(P, array_size, sizeof(double),
			       "P", "ALPHA_realloc()");
...
{
  A = (int *)recalloc_error(A, A_size, sizeof(int),
			    "A", "ALPHA_truncate()");

  if (Comp_flag == 0)
    free_error((char *)A_comp, "A_comp", "ALPHA_truncate()");
  else
    A_comp = (int *)recalloc_error(A_comp, A_size, sizeof(int),
				   "A_comp", "ALPHA_truncate()");

  P = (double *)recalloc_error(P, A_size, sizeof(double),
			       "P", "ALPHA_truncate()");
}
